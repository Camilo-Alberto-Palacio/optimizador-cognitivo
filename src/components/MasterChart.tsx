...
const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label }) => {
  if (active && payload && payload.length > 0) {
    ...
    return (
      <div className="bg-white border border-slate-200 p-3 md:p-4 rounded-3xl shadow-2xl min-w-[160px] md:min-w-[220px]">
        ...
        <div className="bg-indigo-50 p-2 md:p-3 rounded-2xl border border-indigo-100 text-center mb-2 md:mb-3 shadow-inner">
          <p className="text-[8px] md:text-[9px] text-indigo-500 font-black uppercase tracking-widest mb-0.5 md:mb-1">CI Real</p>
          <p className="text-2xl md:text-4xl font-black text-indigo-700 leading-none">{dp.iq}</p>
        </div>
        <div className="flex justify-between text-[9px] md:text-[10px] font-bold px-1 text-slate-400">
          <span>{dp.optimized}%</span>
